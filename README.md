<h1 align="center">Приветствую, меня зовут Володин Сергей 👋</h1>
<h3 align="center">Разработчик Bitrix24</h3>
<p align="center">Создаю инструменты для работы с данными: отчёты, дашборды и интеграции, которые помогают бизнесу принимать решения на основе данных.</p>
<hr>

<h2>Обо мне</h2>

<p>
Frontend-разработчик с <b>4+ годами коммерческого опыта</b> .
</p>

<p>
Последний год специализируюсь на разработке аналитических инструментов, дашбордов и интеграций для Bitrix24.
</p>

<p>
Работал над проектами для:
</p>

<div align="center">

<table>
<tr>
<td align="center" width="120">
<a href="https://alfabank.ru" target="_blank">
  <img src="media/alfa-bank.icon.png" width="50"><br>
  <sub><b>Альфа-Банк</b></sub>
</a>
</td>

<td align="center" width="120">
<a href="https://indrive.com" target="_blank">
  <img src="media/indrive-icon.svg" width="50"><br>
  <sub><b>InDrive</b></sub>
</a>
</td>

<td align="center" width="120">
<a href="https://splat.global" target="_blank">
  <img src="media/splat-global.icon.png" width="50"><br>
  <sub>
    <b>Splat Global</b></sub>
</a>
</td>

</tr>
</table>

</div>

<hr>

<h2>Чем занимаюсь</h2>
<ul>
<li>📊 Разработка аналитических отчётов и дашбордов для Bitrix24 </li>
<li>🔄 Интеграции CRM с внешними сервисами (REST API) </li>  
<li>🧠 Обработка и сегментация CRM-данных по бизнес-логике</li>
<li> 🎯 Создание интерфейсов под задачи бизнеса  </li>
</ul>

<hr>

<h2>Технологии</h2>

<div >

<h3>Frontend</h3>
<table>
<tr>
<td align="center" width="110">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vuejs/vuejs-original.svg" width="45"/><br>
<sub><b>Vue 3</b></sub>
</td>

<td align="center" width="110">
<img src="https://nuxt.com/assets/design-kit/icon-green.svg" width="45"/><br>
<sub><b>Nuxt 4</b></sub>
</td>

<td align="center" width="110">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" width="45"/><br>
<sub><b>TypeScript</b></sub>
</td>

<td align="center" width="110">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="45"/><br>
<sub><b>JavaScript</b></sub>
</td>
</tr>
</table>

<h3>Сборка и тестирование</h3>

<table>
<tr>
<td align="center" width="110">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vitejs/vitejs-original.svg" width="45"/><br>
  <sub><b>Vite</b></sub>
</td>

<td align="center" width="110">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vitest/vitest-original.svg" width="45"/><br>
  <sub><b>Vitest</b></sub>
</td>

<td align="center" width="110">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jest/jest-plain.svg" width="45"/><br>
  <sub><b>Jest</b></sub>
</td>
</tr>
</table>


<h3>Интеграции и API</h3>

<p>
Интеграции с внешними сервисами и автоматизация бизнес-процессов
</p>

<table>
<tr>

<td align="center" width="120">
<img src="https://github.com/user-attachments/assets/8d146d5c-1c3e-40e5-af64-584c1aadf25f" width="45"/><br>
<sub><b>Bitrix24</b></sub>
</td>

<td align="center" width="120">
<img src="https://www.unisender.com/favicon.ico" width="45"/><br>
<sub><b>UniSender</b></sub>
</td>

</tr>
</table>

<h3>Контроль версий и репозитории</h3>

<table>
<tr>
  <td align="center" width="110">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="45"/><br>
    <sub><b>Git</b></sub>
  </td>

  <td align="center" width="110">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="45"/><br>
    <sub><b>GitHub</b></sub>
  </td>

  <td align="center" width="110">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/gitlab/gitlab-original.svg" width="45"/><br>
    <sub><b>GitLab</b></sub>
  </td>
</tr>
</table>

</div>
<hr>

<h2>Проекты</h2>

<p>
Примеры реализованных решений и аналитических инструментов для Bitrix24. Список будет пополняться новыми кейсами:
</p>

<ul>

<li>
<b>Контроль трудозатрат и загрузки команды</b> — аналитика распределения задач и выявление перерасхода времени в работе сотрудников.<br>
<a href="https://github.com/volodin7ergey/bitrix24-time-tracking" target="_blank">🔗 Перейти к проекту</a>
</li>

<br>

<li>
<b>Аналитика продаж и прибыли по компаниям</b> — расчет ключевых финансовых показателей и эффективности клиентов за выбранный период.<br>
<a href="https://github.com/volodin7ergey/bitrix24-company-revenue" target="_blank">🔗 Перейти к проекту</a>
</li>

<br>

<li>
<b>Рейтинг сотрудников отдела продаж</b> — сравнение показателей эффективности и визуализация результатов работы менеджеров.<br>
<a href="https://github.com/volodin7ergey/bitrix24-employee-sales-analytics" target="_blank">🔗 Перейти к проекту</a>
</li>

<br>

<li>
<b>Контроль качества данных</b> — выявление неполных и некорректных CRM-записей с фильтрацией по типам и периодам<br>
<a href="https://github.com/volodin7ergey/bitrix24-data-quality-report" target="_blank">🔗 Перейти к проекту</a>
</li>

<br>

<li>
<b>Сегментация клиентской базы и формирование аудиторий для рассылок — отбор клиентов из Bitrix24 по заданным условиям и подготовка сегментов для email-рассылок.<br>
<a href="https://github.com/volodin7ergey/bitrix24-audience-segmentation" target="_blank">🔗 Перейти к проекту</a>
</li>

<br>

<li>
<b>Другие проекты и эксперименты</b> — новые решения и кейсы в разработке.
</li>

</ul>

<p>
Каждый проект демонстрирует подход к решению прикладных задач и работе с данными.
</p>

<hr>

<h2>Контакты</h2>

<p align="center">
Всегда открыт к обсуждению проектов и задач в области Bitrix24.
</p>

<div align="center" style="margin-top: 10px;">
<a href="https://t.me/volodin7ergey" target="_blank">
  
<img src="https://cdn-icons-png.flaticon.com/512/2111/2111646.png" width="50" title="Telegram">

</a>

<a href="https://vk.com/volodin7ergey" target="_blank">
<img src="https://cdn-icons-png.flaticon.com/512/145/145813.png" width="50" title="VK">
</a>
</div>
